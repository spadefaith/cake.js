module.exports = function(dependency){
    ...
    function Cake(name){
        this.name = name;
        this.components = {};
        this.Subscribe = Cake.alloc(Cake.Subscribe, name);
        this.Handlers = Cake.alloc(Cake.Handlers, name);
        this.Models = Cake.alloc(Cake.Models, name);
        this.Attribute = Cake.alloc(Cake.Subscribe, name, new Attrib());
    };

    Cake.alloc = function(store, name, value){
        if (!store[name]){
            store[name] = value || {};
        };
        return store[name];
    ...
    Cake.prototype.Models.$loaded = function(name){
        return new Promise((res, rej)=>{
            let mk = setInterval(()=>{
                if (this.Models[name]){
                    clearInterval(mk);
            ...
            component.fire = (function(){
                function fire(name, variable){
                    ...
                    if (typeof fn == 'function'){
                        ...
                        // console.log({component:component.name, event:name, payload});
