(() => {
  ...
  // src/scripts/storage/router-storage.js
  var require_router_storage = __commonJS({
    "src/scripts/storage/router-storage.js"(exports, module) {
      var routerStorage = {};
      module.exports = {
        set: function(path, config) {
          let name2 = config.name;
          config.path = path;
          routerStorage[name2] = config;
          return true;
        },
        get: function(name2) {
          return routerStorage[name2];
        }
      };
    }
  });
  ...
  var require_data_for = __commonJS({
    "src/scripts/attrib/data-for.js"(exports, module) {
      ...
      var RouterStorage = require_router_storage();
      var _utils = require_utils2();
      var getConfig = _utils.getConfig;
      var updateConfig = _utils.updateConfig;
      var extendConfig = _utils.extendConfig;
  ...
  // src/scripts/attrib/data-route.js
  var require_data_route = __commonJS({
    "src/scripts/attrib/data-route.js"(exports, module) {
      var Utils = require_utils();
      var _utils = require_utils2();
      var getConfig = _utils.getConfig;
      var updateConfig = _utils.updateConfig;
      ...
      module.exports = async function(prop, newValue, prevValue, component2, html) {
        html = html || document;
        let st = this.storage.get(component2, "route");
        let configs = getConfig(st, prop, newValue, prevValue);
        if (!configs) {
          return;
        }
  ...
  var require_attributes = __commonJS({
    "src/scripts/attributes.js"(exports, module) {
      ...
      Attrib.prototype._compileRoute = function(elRoutes, component2, isStatic) {
        return new Promise((res) => {
          this._loopElements("route", elRoutes, component2, isStatic, function(el, id2, target, gr, index) {
            for (let g = 0; g < gr.length; g++) {
              let val = gr[g].split(" ").join("");
              let isFromFor = el.closest("[data-for]");
              this._register(component2, "route", { bind: val, sel: id2, for: !!isFromFor });
            }
            ;
            this.uiid++;
            el.dataset.route = id2;
          }.bind(this));
          res();
        });
      };
        ...
        if (q && q.length) {
          for (let a = 0; a < q.length; a++) {
            ...
            if (item.for == false) {
              let sel = item.sel;
              let bind2 = item.bind;
              let rconfig = RouterStorage.get(bind2);
              if (rconfig) {
